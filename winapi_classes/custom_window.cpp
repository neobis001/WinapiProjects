...
void CSWindow::copy_window_data(const CSWindow& other) {
	...
	row_weights = other.row_weights;
	col_weights = other.col_weights;
...
int CSWindow::get_total_weights(std::vector<int> weight_list) {
	int total_weights = 0;
	std::vector<int>::const_iterator weight_iter;

	for (weight_iter = weight_list.begin(); 
	weight_iter != weight_list.end(); weight_iter++) {
		total_weights += *weight_iter;
	}
...
int CSWindow::get_weighted_space(LONG window_dimension, int dim_coord, 
	int span, std::vector<int> weight_list) {
	int alloted_weight = 0;
	for (int i = dim_coord; i < dim_coord + span; i++) {
		alloted_weight += weight_list.at(i);
	}
	int total_weight = get_total_weights(weight_list);
...
void CSWindow::check_okay_gridding(int row_number, int col_number, int rowspan, int colspan) {
	...
	if (!has_okay_grid_coords(row_number, col_number, rowspan, colspan)) {
		...
		wsprintf(buffer, "Error (in CSWindow::check_okay_gridding): coords out of range: \n"\
			...
			colspan, parent->get_num_rows(), parent->get_num_cols());
...
bool CSWindow::pure_grid_coords_are_valid(int row_number, int col_number) {
	if (row_number < 0 || row_number >= parent->get_num_rows()) {
		return false;
	}
	else if (col_number < 0 || col_number >= parent->get_num_cols()) {
		return false;
	}
	else {
		return true;
...
bool CSWindow::grid_spanning_is_valid(int row_number, int col_number, int rowspan, int colspan) {
	...
	if (rowspan < 1 || row_number + rowspan > parent->get_num_rows()) {
		return false;
	}
	else if (colspan < 1 || col_number + colspan > parent->get_num_cols()) {
		return false;
	}
	else {
		return true;
...
void CSWindow::initialize_gridding_weights(std::vector<int>& weight_list, int num_weight_entries) {
...
int CSWindow::get_weighted_grid_coord(LONG window_dim,
	int dim_coord, std::vector<int> weight_list) {
	int coord_weight = 0;
	for (int i = 0; i < dim_coord; i++) {
		coord_weight += weight_list.at(i);
	}
	int total_weight = get_total_weights(weight_list);
...
void CSWindow::add_child(PCSWindow child) { //idea: append previous children pointers first
	...
	if (in<PCSWindow>(child, children)) {
		DYNCHAR buffer[200];
...
void CSWindow::prg_draw_as_grid(HDWP* hdwp) {
	...
	int parent_number_cols = parent->get_num_cols();
	int parent_number_rows = parent->get_num_rows();
	std::vector<int> parent_row_weights = parent->row_weights;
	std::vector<int> parent_col_weights = parent->col_weights;

	//
	//calculate the dimensions needed to normally grid an item, w/o considering padding
	//or edge cases
	int allocated_width = get_weighted_space(parent_client_width, col_number, columnspan, parent_col_weights);
	int allocated_height = get_weighted_space(parent_client_height, row_number, rowspan, parent_row_weights);

	int target_x = get_weighted_grid_coord(parent_client_width, col_number, parent_col_weights);
	int target_y = get_weighted_grid_coord(parent_client_height, row_number, parent_row_weights);

	//
	//possibly adjust the allocated width/height if it's on an edge row/column
	//(since for example allocated widths may not add up 
	//to fit the entire client area due to uneven division)
...
int CSWindow::get_num_cols() {
	return col_weights.size();
}

int CSWindow::get_num_rows() {
	return row_weights.size();
}
...
void CSWindow::weight_grid_row(int row_num, int weight) {
	if (!weight_changes_are_valid(row_num, get_num_rows(), weight)) {
		DYNCHAR buffer[200];
		wsprintf(buffer, "Error (in CSWindow::weight_grid_row): invalid weight inputs\n"\
			...
			row_num, get_num_rows(), weight);
		msg_force_quit(buffer);
	} else {
		int& index_weight = row_weights.at(row_num);
		index_weight = weight;
	}
}

void CSWindow::weight_grid_col(int col_num, int weight) {
	if (!weight_changes_are_valid(col_num, get_num_cols(), weight)) {
		DYNCHAR buffer[200];
		wsprintf(buffer, "Error (in CSWindow::weight_grid_col): invalid weight inputs\n"\
			"col number: %i, number cols: %i\n"\
			"input weight: %i (should at least be 1 to be a feasible weight)",
			col_num, get_num_cols(), weight);
		msg_force_quit(buffer);
	}
	else {
		int& index_weight = col_weights.at(col_num);
		index_weight = weight;
	}
}
