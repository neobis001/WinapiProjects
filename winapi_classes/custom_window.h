...
private:
	...
	int get_total_weights(std::vector<int> weight_list);
	...
	//given equal spacing
	void initialize_gridding_weights(std::vector<int>& weight_list, int num_weight_entries);
	
	//focus on a dim_coord that's either a row or column number and with the corr. window data
	//return the corr. pixel
	int get_weighted_grid_coord(LONG window_dim, int dim_coord, std::vector<int> weight_list);
...
protected:
	...
	std::vector<PCSWindow> children;
	PCSWindow parent = nullptr; //won't worry about Rule of Three, see comment on destructor below
	HWND hwnd = (HWND)NULL;
	...
	//with initializeing gridding weights, these lists can reflect number rows and cols
	std::vector<int> row_weights; //a list of ints mapping each row with a weight value
	//divide an entry by the sum of weights to get the relative influence/size of the row
	std::vector<int> col_weights; //like row weights, but for columns
...
public:
	...
	//won't worry about Rule of Three
	//just put self-caution on which PCSWindow should be deleted in main
	//and how PCSWindows should be handled between instances
	~CSWindow();
